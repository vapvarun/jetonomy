...
* New: [jetonomy_compose_topic] shortcode + jetonomy/compose-topic block — lets signed-in members start a topic from any WordPress page, post, or page-builder canvas. Two modes: lock to one space via space_id, or show a picker of spaces the current user is a member of and can post in. Logged-out viewers see a sign-in CTA that redirects back to the original URL after login. Works inside Elementor, Divi, Bricks, WPBakery and any other page builder that renders shortcodes outside the_content — assets enqueue on-demand at render time.
* New: GET /jetonomy/v1/spaces?postable_by_me=1 — REST filter that returns only spaces where the current user is a member AND has create_posts permission. Drives the compose-topic picker and is available for integrations.
* Fixed: Share button on single posts now actually opens a share dropdown. The old dropdown was inserted as a sibling of the share button and positioned with CSS bottom:100% — which anchored to .jt-post-foot (the shared parent) instead of the button itself and pushed the menu off-screen at top:-184px. Dropdown is now positioned via JS from the button's viewport coordinates, right-aligned, auto-flips above when there's no room below, and clamps to the viewport on small screens. Social icons are now crisp Lucide SVGs (Copy link, Twitter/X, Facebook, LinkedIn) with hover + keyboard focus states. No more emoji-to-img fallback to worry about.
* Fixed: Post composer no longer silently fails when the body is empty. Previously a title-only submit would quietly reset the button with no feedback — Pro polls extension had the same bug because it filters jetonomy_new_post_submit_action to substitute its own handler. Both the free submitNewPost and Pro submitNewPostWithPoll now surface a visible inline error ("Please enter a title…" / "Please add some details before posting.") and validate against textContent rather than innerHTML so interacted-then-cleared contenteditables don't mask the check.
* Fixed: Editing a post with an uploaded image no longer strips the image. The old inline editor seeded a textarea from bodyEl.innerText which drops all HTML including img tags, so the PATCH sent back plain text and the server silently replaced the content. Editor now uses a contenteditable div seeded from innerHTML — the image is visible while editing and round-trips intact on save. editReply had the same bug, fixed at the same time so replies with embeds/images/formatting survive edits too.
* Changed: Reaction picker no longer depends on wp-emoji-loader.js. The server used to render raw emoji characters (👍 ❤️ 😂 🎉 👀 👎) and rely on WordPress's emoji-to-img swap; on hosts where that swap is blocked or disabled, the raw characters would inherit color:transparent on the picker button and become invisible. Shipping 8 Microsoft Fluent UI Emoji "Color" SVGs (MIT licensed) inline via the reactions extension — rich, 3D-ish, identical on every browser and OS regardless of emoji conversion state. Added ~120 KB to the Pro plugin.
* Changed: Jetonomy block assets now register on both wp_enqueue_scripts AND enqueue_block_editor_assets — a latent bug meant editor_script handles never resolved inside /wp-admin/ so the block inserter couldn't find our blocks. Affects all blocks with an editor script registered.
* Fixed: TikTok rich previews + embeds now work for share-button URLs (Basecamp 9809230457 / Zoho 39463). TikTok's mobile Share button hands out https://www.tiktok.com/t/<code>/ short URLs that WordPress core's hardcoded TikTok provider regex (which only matches the canonical /@user/video/<id> form) silently rejected, so customers pasting the share-sheet URL got a bare "TikTok – Make Your Day" preview card instead of the player. The OEmbed_Provider now resolves recognised short-URL forms once via cached HEAD request before consulting WP's provider list. Same fix wired up for t.co (Twitter), redd.it + reddit.com /r/<sub>/s/<code> (Reddit mobile), fb.watch (Facebook), and spoti.fi (Spotify) so paste-from-app flows on every major platform stop returning generic OG cards. x.com URLs are also normalised to twitter.com inside the provider for direct callers that bypass X_Provider.
* Fixed: Theme blockquote chrome (left border, italic text, tinted background) no longer bleeds onto TikTok / Instagram / Twitter embed wrappers. Affected every theme that ships a default blockquote rule (BuddyX, Reign, Twenty Twenty-* and most third-party themes). Reset is scoped tightly to the provider classes (.tiktok-embed, .instagram-media, .twitter-tweet) inside our content wrappers so site-wide quote styling is untouched.

Upgrading from 1.3.6 does not require any migration; behaviour changes are additive.
