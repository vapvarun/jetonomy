...
- **Free (`jetonomy/v1` core, 22 controllers)**: complete as of 1.9.0 (2026-08-01) — `openapi.json` documents all 77 free route paths, checked by diffing its `paths` keys against `audit/manifest.json`. That check found four routes the spec had drifted past (`/auth/app-connect`, `/spaces/{id}/invites`, `/spaces/{id}/invites/{invite_id}`, `/users`); they are now documented. Re-run that diff after adding any route — the spec does not regenerate itself, and drift here is silent.
