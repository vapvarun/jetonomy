...
> **Do not edit this file.** It is generated from `audit/manifest.json` in the plugin repository (not part of this docs site, so it is named rather than linked) by `bin/gen-hooks-reference.php` and regenerated at every release, so it cannot drift from the code. For explanations and copy-paste examples, use the hand-written [Hooks Reference](./02-hooks-reference.md); when the two disagree on names or arguments, THIS file wins.

Plugin version: 1.9.7 — 106 actions, 126 filters.
