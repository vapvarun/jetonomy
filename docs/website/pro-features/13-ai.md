...
Replaces the free plugin's pattern-based spam detector with a model-driven classifier. Posts and replies publish instantly - the AI is never in the posting path. The background reviewer then scores each new post and reply for spam probability and moves anything above the threshold to the moderation queue. Content below the threshold stays published as normal.
