...
![The AI Integration settings tab: provider activation (OpenAI, Anthropic, Ollama, Custom), the default-provider fallback chain, per-feature toggles for spam detection, content moderation, reply suggestions and thread summaries, the live usage dashboard, member usage limits, and the monthly spend cap](../images/pro-ai-settings.png)
...
AI review never sits between a member and the publish button. Posts and replies appear the instant they are submitted, with zero AI latency, so posting always stays fast. A background job then reviews the new content in batches on a schedule.

- **Batched and scheduled.** A recurring job (`jetonomy_pro_ai_batch_review`) sweeps everything posted since its last run and classifies many items in a single model call - the spam and content-moderation checks share one pass. On a quiet site the sweep exits without touching the API at all.
- **Runs every 5 minutes by default.** The interval is filterable via `jetonomy_pro_ai_review_interval` (default 300 seconds, floored at 60 seconds).
- **Trusted members are skipped.** Staff and members at or above a trust level are never sent to the model. The exemption threshold is filterable via `jetonomy_pro_ai_review_exempt_trust_level` (default trust level 3).
- **Throughput controls.** Each sweep inspects up to `jetonomy_pro_ai_review_batch_cap` new posts and replies (default 60), classifying `jetonomy_pro_ai_review_chunk_size` items per model call (default 12). A failed model call is retried on the next cycle rather than dropped.

Both Spam Detection and Content Moderation below run together in this same background pass.

### Spam Detection

The free plugin already classifies spam with a model rather than with patterns, but it can only talk to a locally hosted Ollama instance and only for that one job. The Pro extension widens that seam: OpenAI, Anthropic and custom providers, an ordered fallback chain when one is unreachable, spend caps, and the same models reused for content moderation, reply suggestions and thread summaries. Posts and replies publish instantly - the AI is never in the posting path. The background reviewer then scores each new post and reply for spam probability and moves anything above the threshold to the moderation queue. Content below the threshold stays published as normal.

Works alongside Akismet and trust-level rate limits - AI spam detection is the third layer, not a replacement.

### Content Moderation

Flags content that breaks rules you describe in plain English. You write a few sentences - "no political attacks, no personal insults, no harassment" - and the background reviewer reads each new post and reply against that policy.
...
Each feature has its own **Enable** switch and an optional **Provider Override**, so you can, for example, run moderation locally on Ollama while sending summaries to OpenAI - see the per-feature toggles in the [AI Integration settings tab](../images/pro-ai-settings.png) above.
...
![The AI Usage widget on the main Jetonomy admin dashboard, showing requests today and this month, token usage by feature, estimated spend by provider with the share of the monthly cap consumed, average response time, and error rate](../images/pro-ai-usage-widget.png)
