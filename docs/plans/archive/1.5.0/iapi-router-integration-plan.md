...
- DONE 5/5 — space-edit (commit c42a078): saveSpace + prefix editor (togglePrefixConfig/addPrefixRow/removePrefixRow) -> store actions; reuses shared cover actions; icon-picker re-inits on jetonomy:navigated; deleted space-edit.js. ALL per-route page scripts now gone. BUG caught + fixed in test: removePrefixRow must use triggerOf(event) not getElement() (hydrator-wired rows have no IA render scope). Verified: client-nav edit-space, add/remove prefix row, save PATCH 200.

GUARD-REMOVAL STATUS: the allow-list is NOT yet deleted. All 5 per-route jetonomy scripts are eliminated, BUT post (/t/) and new-post still ship heavy IMPERATIVE classic assets that don't re-init on client nav: vendor prismjs (code highlight) + composer.js (binds the editor/mention/instant-search on load). They stay full-load via the guard. To finish (delete the guard so EVERY route client-navs): (1) make prismjs global (already chosen) or re-highlight on jetonomy:navigated; (2) make composer.js idempotent + re-init on jetonomy:navigated (900-line file — its own careful pass); then drop post/new-post from the guard and remove it entirely. This is the true final step.

- DONE 4/5 — new-space (commit 689dc8c): createSpace + SHARED uploadCover/removeCover/jtSetCoverPreview (reused by edit-space), deleted new-space.js. Verified: create -> POST /spaces 201 + redirect. NOT yet in allow-list (icon-picker still classic load-once; client-nav for the form routes lands with 5/5).
- DONE 3/5 — notifications-page (commit 49a086e): mark-all/read/delete/bulk/select-all -> store actions; menu reuses jetonomySmartDropdown; deleted notifications-page.js; /notifications/ in allow-list. Verified: client-nav in, bulkbar, menu, mark-read PATCH 200, qa-actions 231/231.
- DONE 2/5 — moderation (commit 3318c55): consolidated. Deleted DEAD dismissFlag action + classic moderation.js; new general actions.resolveFlag (keeps the per-card data-resolve-endpoint for space-scoped queues, canonical valid/dismissed vocabulary). flag-card.php wired; /mod/ + /s/{slug}/mod/ added to allow-list. Verified: dismiss -> POST 200, card removed, /mod/ client-navs, qa-actions 231/231.
