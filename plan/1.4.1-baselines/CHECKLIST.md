...
| A7 | Revisions admin page | ✅ DONE | 2026-04-29 | New `?page=jetonomy-revisions` admin page (sidebar slot between Activity Log and Users — order Content · Moderation · Activity Log · Revisions · Users preserved). `Jetonomy\Admin\Revisions_List_Table` extends `WP_List_Table` (read-only); list mode aggregates `(object_type, object_id)` pairs with columns Type · Title (linked) · Revisions · Last Edited · Last Edited By; filters by type + date range; default 20/page. Detail mode (`?object_type=…&object_id=…`) renders per-object revision list with WP-core `wp_text_diff()` left/right pane per row. Two new model helpers: `Revision::list_objects_with_revisions()` + `count_objects_with_revisions()` (correlated subquery for last-editor). N+1 avoided via batched IN() title prelookup. Capability: `jetonomy_manage_settings`; explicit `wp_die()` guard at the top of `render_revisions_page()` so direct hits fail closed (subscriber → 403). PHPStan baseline `JETONOMY_DIR` count bumped 14→16 for the two new include statements (matches A6's pattern). qa-actions 210/210 PASS; access-matrix 78/78 PASS (drift unchanged from A4/A5 — A7 makes no REST changes); all 11 admin pages still <50ms. |
| A8 | Email Templates admin editor | ✅ DONE | 2026-04-29 | Re-scoped: editor already existed in Settings → Notifications. A8 enhances in place — no new admin page (avoids further +submenu after A6/A7). New `Notifier::get_default_template()` helper centralizes shipped subject/body for all 11 notification types (single source of truth — both seed sites in `class-jetonomy.php` now delegate; bit-identical migration). Sanitizer `$allowed_types` + view `$tmpl_types` + `Settings_Handler::sample_fixtures()` all extended with `verification_reminder` (was previously seeded but unreachable from the editor). New per-row "Reset to default" button rendered only when an override is persisted (`isset($email_templates[$type])` guard); confirms with `confirm()`, POSTs `wp_ajax_jetonomy_email_reset` (nonce `jetonomy_admin` + cap `jetonomy_manage_settings`), `unset()`s the row from the option, returns defaults so JS repopulates inputs in place. Server-side smoke: all 11 types resolve, unknown types rejected, sanitizer strips `<script>` via `wp_kses_post`, reset removes only the targeted key (other rows untouched). qa-actions 210/210 PASS; access-matrix 78/78 PASS (REST surface unchanged — drift carried from A4/A5 unchanged, A6/A7 noted same). Commits: A8.1 `f1ec0e5`, A8.2 `ad32133`, A8.3 `8360994`. |
