...
class BuddyPress {
	...
	public function enqueue_styles(): void {
		...
		// The token layer is a DEPENDENCY, not an optional extra. buddypress.css
		// consumes `--jt-*` and declares none of them, so without this the whole
		// set resolves to nothing on every BuddyPress page: the member "Forum"
		// tab rendered light inside a dark theme, with body text at 1.12:1
		// against the background. This is the same omission
		// jetonomy_register_token_style()'s docblock was written about after the
		// 1.9.3 space-delete dialog shipped invisible.
		wp_enqueue_style(
			'jetonomy-buddypress',
		...
		// buddypress-rtl.css was generated by `grunt rtlcss` from the first
		// commit and never enqueued by anything, so RTL sites got the LTR sheet
		// - mirrored-wrong stats and tags on every Arabic, Hebrew or Persian
		// community. Same is_rtl() shape the main stylesheet uses in
		// Template_Loader, so there is one convention rather than two. The sheet
		// now carries almost nothing, because buddypress.css was converted to
		// logical properties in the same change and the browser flips those for
		// free; it stays enqueued for whatever asymmetric values appear later.
		if ( is_rtl() ) {
			wp_enqueue_style(
				'jetonomy-buddypress-rtl',
				JETONOMY_URL . 'assets/css/buddypress-rtl.css',
				array( 'jetonomy-buddypress' ),
				JETONOMY_VERSION
			);
		}
	}
