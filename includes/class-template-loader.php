	...
	public static function render( array $data ): void {
		...
		wp_interactivity_state(
			...
			'i18n'              => array(
				...
				// 1.4.4 i18n sweep (#9876871333): keys consumed by view.js
				// state.i18n? fallbacks that had no provider so far. Each one
				// is paired with the exact English string the JS used as its
				// hard-coded fallback so translated locales now render the
				// same copy in their language instead of falling through to
				// English.
				...
				'failedClose'           => __( 'Failed to toggle close.', 'jetonomy' ),
				'postClosed'            => __( 'Topic closed', 'jetonomy' ),
				'postReopened'          => __( 'Topic reopened', 'jetonomy' ),
				...
				'statusCleared'         => __( 'Removed from roadmap', 'jetonomy' ),
				'unaccepted'            => __( 'Reply unaccepted', 'jetonomy' ),
		...
		wp_localize_script(
			...
			array(
				...
				'i18n'          => array(
					...
					// 1.4.4 i18n sweep (#9876871333): composer.js join-space + request-to-join
					// fallback toasts. The REST response usually carries a translated `message`
					// from the server, but on network failure / unstructured response the JS
					// falls back to these client-side strings — they need to translate too.
					'couldNotJoinSpace'        => esc_html__( 'Could not join space.', 'jetonomy' ),
					'requestSubmittedAwaiting' => esc_html__( 'Request submitted. Awaiting approval.', 'jetonomy' ),
					'couldNotSubmitRequest'    => esc_html__( 'Could not submit request.', 'jetonomy' ),
				),
			)
		);
