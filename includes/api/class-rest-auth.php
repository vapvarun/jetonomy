...
class REST_Auth {
	...
	/**
	 * Build a permission_callback for a read route.
	 *
	 * Allows logged-out access by default (matches WP REST defaults for GETs).
	 * If `$caps` is supplied AND the user is logged in, the caps are checked;
	 * logged-out users still pass without a cap check so anonymous reads
	 * remain anonymous-friendly. Pass `null` (default) for unconditional
	 * read access.
	 *
	 * @since 1.4.3
	 *
	 * @param string|string[]|null $caps Optional capability/caps to require
	 *                                   for logged-in users.
	 * @return Closure(WP_REST_Request): (true|WP_Error)
	 */
	public static function auth_read( $caps = null ): Closure {
		return static function ( WP_REST_Request $request ) use ( $caps ) {
			unset( $request );
			...
			// Logged-out users get unconditional read access — that's the
			// whole point of `auth_read`. Cap checks only kick in once the
			...
			return new WP_Error(
				'rest_forbidden',
				__( 'You do not have permission to access this resource.', 'jetonomy' ),
				array( 'status' => 403 )
			);
		};
	}
