...
class Demo_Seeder {
	...
	public static function seed( int $admin_id ): array {
		...
		$demo = [ 'users' => [], 'categories' => [], 'spaces' => [], 'posts' => [], 'replies' => [], 'flags' => [], 'polls' => [], 'badges' => [] ];
		...
		$s_welcome = Space::create( [
			'category_id' => $cat2, 'author_id' => $admin_id, 'type' => 'forum',
			'title'       => 'Welcome & Introductions', 'slug' => 'welcome',
		...
		$s_general = Space::create( [
			'category_id' => $cat2, 'author_id' => $alice, 'type' => 'forum',
			'title'       => 'General Discussion', 'slug' => 'general-discussion',
		...
		$s_help = Space::create( [
			'category_id' => $cat1, 'author_id' => $admin_id, 'type' => 'qa',
		...
		$s_ideas = Space::create( [
			'category_id' => $cat1, 'author_id' => $alice, 'type' => 'ideas',
		...
		$s_tips = Space::create( [
			'category_id' => $cat1, 'author_id' => $admin_id, 'type' => 'forum',
			'title'       => 'Tips & Best Practices', 'slug' => 'tips-best-practices',
		...
		$posts_raw = [
			[ 'space' => $s_welcome, 'type' => 'topic',    'author' => $admin_id, 'title' => "Welcome to our community — here's how it works", 'content' => "<p>Hey everyone! We're excited to have you here.</p><p>This community is built for real conversations — no algorithms, no noise. Here's a quick orientation:</p><ul><li><strong>Spaces</strong> are topic-specific areas. Browse the ones that interest you and join freely.</li><li><strong>Reputation</strong> grows as you contribute. Higher trust levels unlock more features.</li><li><strong>Voting</strong> helps the best content rise to the top. Use it generously.</li></ul><p>Don't be shy — introduce yourself below or jump straight into a discussion. Welcome aboard!</p>" ],
			[ 'space' => $s_welcome, 'type' => 'topic',    'author' => $admin_id, 'title' => 'Community guidelines — the short version', 'content' => '<p>We keep things simple. Three principles:</p><ol><li><strong>Be respectful.</strong> Disagree with ideas, not with people. No personal attacks.</li><li><strong>Be helpful.</strong> If someone asks a question, try to answer it — or point them in the right direction.</li><li><strong>Stay on topic.</strong> Each space has a purpose. Use General Discussion for everything else.</li></ol><p>Moderators are here to keep conversations productive. If you see something that doesn\'t belong, use the flag button.</p>' ],
			[ 'space' => $s_general, 'type' => 'topic',   'author' => $alice,    'title' => "What's everyone working on this week?", 'content' => "<p>I always find it motivating to hear what others are building. I'm currently migrating a client's legacy forum to this platform — the import tools have been surprisingly smooth.</p><p>What's on your plate? Drop a quick update below.</p>" ],
			[ 'space' => $s_general, 'type' => 'topic',   'author' => $bob,      'title' => 'Interesting article: the future of online communities', 'content' => "<p>Came across a thoughtful piece about how community platforms are shifting away from engagement metrics toward meaningful interactions. The core argument is that smaller, focused communities consistently outperform large social networks for professional learning.</p><p>Curious what you all think — does that match your experience?</p>" ],
			[ 'space' => $s_help,    'type' => 'question', 'author' => $carol,    'title' => 'How do I customize the notification settings?', 'content' => "<p>I'm getting email notifications for every reply in spaces I've joined. Is there a way to set it to daily digest instead? I looked in my profile settings but couldn't find the option.</p><p>Running the latest version on WordPress 6.9 with the BuddyX theme.</p>" ],
			[ 'space' => $s_help,    'type' => 'question', 'author' => $david,    'title' => 'Can I restrict a space to specific membership levels?', 'content' => '<p>We have a premium membership tier using MemberPress. I want to create a space that\'s only visible to members at the "Pro" level and above.</p><p>I see there\'s an Access Rules section in the space settings — is that the right place? What should the configuration look like?</p>' ],
			[ 'space' => $s_help,    'type' => 'question', 'author' => $bob,      'title' => 'Best approach for migrating from bbPress?', 'content' => '<p>We have about 3,000 topics and 12,000 replies in bbPress. Before I hit the import button, a few questions:</p><ul><li>Does the importer preserve the original post dates?</li><li>What happens to forum categories — do they become spaces?</li><li>Is there a way to do a dry run first?</li></ul><p>Any migration tips from people who\'ve done this would be really helpful.</p>' ],
			[ 'space' => $s_ideas,   'type' => 'idea',     'author' => $carol,    'title' => 'Dark mode toggle in user preferences', 'content' => "<p>It would be great if users could switch to a dark color scheme directly from their profile preferences, independent of the system theme. Many of us work late and a dark mode would reduce eye strain significantly.</p><p>Ideally it should respect the theme's dark palette if one exists, and fall back to a sensible default otherwise.</p>" ],
			[ 'space' => $s_ideas,   'type' => 'idea',     'author' => $alice,    'title' => 'Saved/bookmarked posts for quick reference', 'content' => '<p>I often find great answers in Q&A threads but have no way to save them for later. A simple "bookmark" or "save" button on posts and replies would be incredibly useful.</p><p>Bonus points if there\'s a "My Saved" page where I can see all my bookmarks organized by space.</p>' ],
			[ 'space' => $s_tips,    'type' => 'topic',    'author' => $alice,    'title' => 'Setting up your community for the first 100 members', 'content' => "<p>After launching three communities over the past two years, here's what I've learned about the critical first 100 members:</p><ol><li><strong>Seed the content yourself.</strong> Nobody wants to post in an empty forum. Write 10–15 quality topics across different spaces before inviting anyone.</li><li><strong>Personal invitations beat mass emails.</strong> Send individual messages to people you know will contribute.</li><li><strong>Respond to everything.</strong> For the first month, reply to every single post. People come back when they feel heard.</li><li><strong>Celebrate first-time posters.</strong> A simple \"Great first post, welcome!\" goes a long way.</li></ol><p>The goal isn't growth — it's establishing the culture. Get the first 100 right and the next 1,000 takes care of itself.</p>" ],
			[ 'space' => $s_tips,    'type' => 'topic',    'author' => $bob,      'title' => 'Keyboard shortcuts you might not know about', 'content' => '<p>Quick productivity tip — this platform has built-in keyboard shortcuts:</p><ul><li><code>j</code> / <code>k</code> — Navigate between topics</li><li><code>l</code> — Upvote the current topic</li><li><code>r</code> — Open the reply composer</li><li><code>n</code> — New post</li><li><code>/</code> — Focus the search bar</li><li><code>?</code> — Show the full shortcut help</li></ul><p>Try pressing <code>?</code> anywhere on the community pages to see the complete list.</p>' ],
		];
		...
		$replies_raw = [
			...
			[ 'post' => 0, 'author' => $alice, 'content' => "<p>This is exactly the kind of community space I've been looking for. Clean interface, no distractions. Happy to be here!</p>" ],
			[ 'post' => 0, 'author' => $bob,   'content' => "<p>Love the reputation system — it's a smart way to build trust gradually. Looking forward to contributing.</p>" ],
			[ 'post' => 0, 'author' => $carol, 'content' => "<p>Just joined today. Coming from a Discourse community that got too noisy. This feels much more focused already.</p>" ],
			// Guidelines (post 1) — david, eve.
			[ 'post' => 1, 'author' => $david, 'content' => '<p>Simple and clear — the best kind of community guidelines. Bookmarked for reference.</p>' ],
			[ 'post' => 1, 'author' => $eve,   'content' => "<p>Appreciate that flagging is encouraged. In my experience that's the best way to keep forums healthy without over-moderating.</p>" ],
			// What are you working on? (post 2) — carol, david, bob.
			[ 'post' => 2, 'author' => $carol, 'content' => "<p>Currently setting up a knowledge base for our support team. We're using the Q&A space type which is perfect for structured answers. The voting system helps surface the best solutions.</p>" ],
			[ 'post' => 2, 'author' => $david, 'content' => "<p>Building out a members-only community for our online course. The MemberPress integration was a game-changer — took about 10 minutes to set up space-level access rules.</p>" ],
			[ 'post' => 2, 'author' => $bob,   'content' => "<p>Rebuilding our company intranet forum. We had bbPress before and the migration import handled 8,000+ posts without a hitch. Pretty impressed so far.</p>" ],
			// Notification Q&A (post 4) — alice (accepted), bob.
			[ 'post' => 4, 'author' => $alice, 'accepted' => true, 'content' => '<p>Go to your profile → Edit → scroll down to the <strong>Notification Preferences</strong> section. You can choose between instant, daily digest, and weekly digest for each type of notification.</p><p>If the section isn\'t visible, make sure you\'re running at least version 1.0. The email digest feature requires Pro.</p>' ],
			[ 'post' => 4, 'author' => $bob,   'content' => '<p>Adding to the above — you can also unsubscribe from individual spaces by clicking the bell icon on the space page. That way you only get notifications for spaces you actively follow.</p>' ],
			// Access rules Q&A (post 5) — alice (accepted), carol.
			[ 'post' => 5, 'author' => $alice, 'accepted' => true, 'content' => "<p>Yes, Access Rules is the right section. Here's the setup:</p><ol><li>Edit the space → Access Rules tab</li><li>Click \"Add Rule\"</li><li>Set Type to \"Membership\", Level to \"Pro\"</li><li>Set the space visibility to \"Private\"</li></ol><p>Members at the Pro level and above will see the space automatically. Others won't even know it exists.</p>" ],
			[ 'post' => 5, 'author' => $carol, 'content' => "<p>One thing to note — if you're using the PMPro adapter instead of MemberPress, the setup is identical. The adapter pattern means all membership plugins work the same way from the community side.</p>" ],
			// bbPress migration (post 6) — alice, david.
			[ 'post' => 6, 'author' => $alice, 'content' => '<p>I migrated about 5,000 topics last week. To answer your questions:</p><ul><li>Yes, original dates are preserved. Posts appear in the correct chronological order.</li><li>bbPress forums become spaces; forum categories become Jetonomy categories.</li><li>Use the CLI command <code>wp jetonomy import bbpress --dry-run</code> to preview without actually importing.</li></ul><p>Tip: run a database backup first. The importer is non-destructive but better safe than sorry.</p>' ],
			[ 'post' => 6, 'author' => $david, 'content' => "<p>Did the same migration last month. The batched import was a lifesaver — we have 12,000 replies and it handled them in chunks with a progress bar. No timeouts. Took about 4 minutes total.</p>" ],
			// Dark mode idea (post 7) — bob, alice.
			[ 'post' => 7, 'author' => $bob,   'content' => "<p>Fully support this. A lot of developer communities default to dark mode now. It would be great to see it built into the user preferences rather than relying on browser extensions.</p>" ],
			[ 'post' => 7, 'author' => $alice, 'content' => "<p>If the theme supports a dark palette via theme.json, would it make sense to just toggle the CSS custom properties? That way it stays consistent with the overall site design.</p>" ],
			// Bookmarks idea (post 8) — david.
			[ 'post' => 8, 'author' => $david, 'content' => "<p>Yes please! I find myself copying URLs into a note-taking app which is not ideal. A native bookmark system would save me so much time. Especially in Q&A spaces where the accepted answers are gold.</p>" ],
			// First 100 members (post 9) — carol, eve.
			[ 'post' => 9, 'author' => $carol, 'content' => "<p>Point 3 is so true. Early on, I was the only person replying in our community. It felt slow, but within a month people started replying to each other. That transition from \"founder answers everything\" to \"community helps itself\" is magical when it happens.</p>" ],
			[ 'post' => 9, 'author' => $eve,   'content' => "<p>Great advice. I'd add one more: <strong>create rituals</strong>. A weekly \"What are you working on?\" thread or a monthly AMA gives people a reason to come back regularly. Consistency beats novelty.</p>" ],
		];
		...
		if ( $wpdb->get_var( "SHOW TABLES LIKE '{$badges_t}'" ) ) { // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
			$badge_defs = [
				...
				[ 'name' => 'Veteran',                'description' => 'Active member for over 30 days.',                           'icon' => 'shield',  'tier' => 'bronze', 'criteria_type' => 'days_active', 'criteria_value' => 30 ],
			...
			if ( $wpdb->get_var( "SHOW TABLES LIKE '{$user_badges_t}'" ) && ! empty( $demo['badges'] ) ) { // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
				$award = static function( int $uid, int $badge_id ) use ( $wpdb, $user_badges_t, $now ) {
					$wpdb->insert( $user_badges_t, [ 'user_id' => $uid, 'badge_id' => $badge_id, 'awarded_at' => $now ] );
				};
	...
	private static function seed_pro( int $admin_id, array $u, array $demo, string $now ): array {
		...
		if ( $wpdb->get_var( "SHOW TABLES LIKE '{$reactions_t}'" ) ) { // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
			...
			if ( $welcome_post ) {
				foreach ( [
					...
					$wpdb->insert( $reactions_t, [ 'user_id' => $uid, 'object_type' => $ot, 'object_id' => $oid, 'emoji' => $emoji, 'created_at' => $now ] );
				}
			}
		...
		if ( $have_polls && $dark_post ) {
			...
			foreach ( $option_labels as $i => $label ) {
				$wpdb->insert( $options_t, [ 'poll_id' => $poll_id, 'label' => $label, 'sort_order' => $i, 'vote_count' => 0 ] );
				$option_ids[] = (int) $wpdb->insert_id;
			}

			// admin→1, alice→0, bob→1, carol→0, david→2. Eve doesn't vote.
			foreach ( [ $admin_id => 1, $alice => 0, $bob => 1, $carol => 0, $david => 2 ] as $voter => $opt_idx ) {
				if ( ! isset( $option_ids[ $opt_idx ] ) ) {
					continue;
				}
				$opt_id = $option_ids[ $opt_idx ];
				$wpdb->insert( $pvotes_t, [ 'poll_id' => $poll_id, 'option_id' => $opt_id, 'user_id' => $voter, 'created_at' => $now ] );
