...
class Privacy {
    ...
    public function export_profile( string $email, int $page = 1 ): array {
        ...
        if ( $profile ) {
            $data[] = [
                ...
                'data'        => [
                    [ 'name' => __( 'Display Name', 'jetonomy' ), 'value' => $profile->display_name ?? '' ],
                    [ 'name' => __( 'Bio', 'jetonomy' ), 'value' => $profile->bio ?? '' ],
                    [ 'name' => __( 'Trust Level', 'jetonomy' ), 'value' => (string) $profile->trust_level ],
                    [ 'name' => __( 'Reputation', 'jetonomy' ), 'value' => (string) $profile->reputation ],
                    [ 'name' => __( 'Post Count', 'jetonomy' ), 'value' => (string) $profile->post_count ],
                    [ 'name' => __( 'Reply Count', 'jetonomy' ), 'value' => (string) $profile->reply_count ],
                ],
            ];
        }
    ...
    public function export_posts( string $email, int $page = 1 ): array {
        ...
        foreach ( $posts as $post ) {
            $data[] = [
                ...
                'data'        => [
                    [ 'name' => __( 'Title', 'jetonomy' ), 'value' => $post->title ],
                    [ 'name' => __( 'Content', 'jetonomy' ), 'value' => $post->content ],
                    [ 'name' => __( 'Date', 'jetonomy' ), 'value' => $post->created_at ],
                ],
            ];
        }

        return [ 'data' => $data, 'done' => count( $posts ) < $limit ];
    }
    ...
    public function export_replies( string $email, int $page = 1 ): array {
        ...
        foreach ( $replies as $reply ) {
            $data[] = [
                ...
                'data'        => [
                    [ 'name' => __( 'Content', 'jetonomy' ), 'value' => $reply->content ],
                    [ 'name' => __( 'Date', 'jetonomy' ), 'value' => $reply->created_at ],
                ],
            ];
        }

        return [ 'data' => $data, 'done' => count( $replies ) < $limit ];
    }
    ...
    public function on_user_delete( int $user_id ): void {
        ...
        $tables = [ 'user_profiles', 'notifications', 'subscriptions', 'read_status',
                    'space_members', 'votes', 'user_interests', 'activity_log',
                    'restrictions', 'join_requests' ];
